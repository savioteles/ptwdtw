...
__global__ void DtwCostMatrixGlobalMemory (float *weighted_local_cost_matrix, float *result, int num_rows, int num_cols, float *cost) {
...
void ComputeDTWMatrix(float *weighted_local_cost_matrix,
	...
	int num_threads = min2(patterns_size, timeseries_size);
