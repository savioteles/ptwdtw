...
src/gpu/%.o: ../src/gpu/%.cpp
	...
	/usr/local/cuda-8.0/bin/nvcc -G -g -O0 -std=c++11 -gencode arch=compute_60,code=sm_60  -odir "src/gpu" -M -o "$(@:%.o=%.d)" "$<"
...
src/gpu/%.o: ../src/gpu/%.cu
	...
	@echo 'Invoking: NVCC Compiler'
	/usr/local/cuda-8.0/bin/nvcc -G -g -O0 -std=c++11 -gencode arch=compute_60,code=sm_60  -odir "src/gpu" -M -o "$(@:%.o=%.d)" "$<"
	/usr/local/cuda-8.0/bin/nvcc -G -g -O0 -std=c++11 --compile --relocatable-device-code=false -gencode arch=compute_60,code=compute_60 -gencode arch=compute_60,code=sm_60  -x cu -o  "$@" "$<"
	@echo 'Finished building: $<'
	@echo ' '
